     ...
     if @sort_type == "joke"
      ...
      when "newest"
       ...
       if @sort == "most"
         ...
         case @time
         when "today"
           user_jokes = Joke.select("user_id, sum((up_votes - down_votes)) as total_votes").where(['user_id is not null and created_at BETWEEN ? AND ?', Date.today, Date.tomorrow - 1.minute]).group("user_id").having("count(id) > 0").sort_by{|x| (sort_direction*x.total_votes.to_i())}.reverse[0...10]
         when "week"
           user_jokes = Joke.select("user_id, sum((up_votes - down_votes)) as total_votes").where(['user_id is not null and created_at BETWEEN ? AND ?', Time.now.beginning_of_week, Time.now]).group("user_id").having("count(id) > 0").sort_by{|x| (sort_direction*x.total_votes.to_i())}.reverse[0...10]
         when "month"
